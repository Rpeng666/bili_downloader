...
impl VideoDownloader {
    ...
    async fn download_dash(&self, video_info: &crate::parser::models::VideoInfo) -> Result<()> {
        ...
        let video_path = self.output_dir.join(format!("tmp/{}_video.mp4", video_info.bvid));
        self.download_file(&video_info.video_url, &video_path).await?;

        // 下载音频流
        println!("------------------------------------------------------");
        println!("开始下载音频流: {}", video_info.audio_url);
        let audio_path = self.output_dir.join(format!("tmp/{}_audio.m4a", video_info.bvid));
        self.download_file(&video_info.audio_url, &audio_path).await?;

        // 合并视频和音频
        println!("------------------------------------------------------");
        println!("开始合并视频和音频...");
        let output_path = self.output_dir.join(format!("output/{}.mp4", video_info.title));
        let merger = MediaMerger;
        merger.merge_av(&video_path, &audio_path, &output_path).await?;
    ...
    async fn download_flv(&self, video_info: &crate::parser::models::VideoInfo) -> Result<()> {
        ...
        self.download_file(&video_info.video_url, &output_path).await?;
        println!("下载完成！");
        Ok(())
    }
